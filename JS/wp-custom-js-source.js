...
(function ($) {
	...
	function HexagonalButton($fromElem) {
		...
		this.vertices = [
			{
				x: validArg ? (this.origin.x + this.apotherm) : undefined,
				y: validArg ? this.origin.y : undefined
			},
			{
				x: validArg ? (this.origin.x + this.apotherm * 2) : undefined,
				y: validArg ? (this.origin.y + tan30 * this.apotherm) : undefined
			},
			{
				x: validArg ? (this.origin.x + this.apotherm * 2) : undefined,
				y: validArg ? (this.origin.y + 3 * tan30 * this.apotherm) : undefined
			},
			{
				x: validArg ? (this.origin.x + this.apotherm) : undefined,
				y: validArg ? (this.origin.y + 4 * tan30 * this.apotherm) : undefined
			},
			{
				x: validArg ? this.origin.x : undefined,
				y: validArg ? (this.origin.y + 3 * tan30 * this.apotherm) : undefined
			},
			{
				x: validArg ? this.origin.x : undefined,
		...
		this.keySegments = [
			{
				slope: validArg ? -tan30 : undefined,
				intercept: validArg ? this.vertices[5].y : undefined
			},
			{
				slope: validArg ? tan30 : undefined,
				intercept: validArg ? this.vertices[4].y : undefined
			},
			{
				slope: validArg ? tan30 : undefined,
				intercept: validArg ? this.vertices[0].y : undefined
			},
			{
				slope: validArg ? -tan30 : undefined,
				intercept: validArg ? this.vertices[3].y : undefined
			}
		];
	}

	HexagonalButton.prototype.getSegmentY(whichSgmnt, pageX) {
		var pageY = undefined;
	...
	HexagonalButton.prototype.isWithinArea(pageX, pageY) {
		var result = false;
